...
imdb_data=imdb_data.drop(['certificate','votes','directors','stars','runtime','produce_year'], axis=1)
...
#data for mapping
topic_list=['Action','Comedy','Crime','Drama','Adventure','Horror','Mystery','Family','Fantasy','Thriller','Romance','Sci-Fi']
feelings=['Joy','Sadness','Surprise','Love','Joy','Fear','Surprise','Love','Joy','Surprise','love','Joy']
...
#%%
#Comparision with genres basis our classsification
...
model, topics,topic_assignments = model_topics(feature_vecs,vectorizer,n_topics=32,n_words=3,model_type='lda')
...
Final_df = Final_df.sort_values(["Count","MetaS","imdbr","score"], ascending = [False,False,False,True])
...
Recommendations=Recommendations.merge(imdb_data,how="left",left_on='Titles',right_on='title_x')
Recommendations=Recommendations.drop(['title_x','title_href','mc_link','genre0','genre1','genre2','introduction','title_y','wiki_link'], axis=1)
...
tokens_cus = [token for token in tokens_cus if token.lower() not in stopwords and token not in punctuations]
#print((tokens)) 
